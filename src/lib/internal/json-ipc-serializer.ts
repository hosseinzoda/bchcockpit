import { hexToBin, binToHex } from '@cashlab/common/util.js';
import { BaseException } from '@cashlab/common/exceptions.js';
...
export const deserialize = (input: string): any => {
  return deserializeMessage(JSON.parse(input));
};

export const serialize = (data: any): string => {
  return JSON.stringify(serializeMessage(data));
};
